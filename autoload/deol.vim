...
function deol#_print_error(string, name = 'deol') abort
  echohl Error
  for line in
...
function deol#_new(cwd, options) abort
  ...
  call s:set_editor(deol.options.nvim_server)
...
function deol#_get_start_insert(tabnr) abort
  const deol = deol#_get(a:tabnr)
  return deol is v:null ? v:null : deol.options.start_insert
endfunction
...
function s:check_options(options) abort
  ...
  for key in a:options->keys()
    if default_keys->index(key) < 0
      call deol#_print_error('Invalid option: ' .. key)
    endif
  endfor
endfunction

function s:set_editor(nvim_server) abort
  " Set $EDITOR.
  let editor_command = ''
